...
**给 Codex Desktop 一键换肤：一张图片就是一个主题，顶部菜单即时切换深浅外观，安装包内还自带可选的 Miku Future Pet。**

*A local CDP-based skin switcher for Codex Desktop on macOS and Windows, with explicit next-launch persistence control.*
...
*真机截图：Miku 488137 高精度主题，顶部中间的「主题」入口可以直接打开主题中心。*

![真机截图：HeiGe 主题中心](docs/images/theme-center-live.webp)

*真机截图：主题中心集中展示当前主题、自定义图片、原生界面、内置主题预览和皮肤常驻开关，主题卡片即点即换。*

主题中心同时显示当前 Skin Studio 版本。只有用户主动点击「检查更新」时，控制器才会访问项目的 GitHub 最新正式 Release；发现新版后可一键复制完整更新指令，粘贴到 Codex 对话中执行。检查功能不会后台联网，也不会自行下载、覆盖文件或重启 Codex。
...
*真机截图：原神 · 破晓 轻量主题（浅色场景底图 + 自动配色），配合 Codex 浅色外观。*

![真机截图：原神星夜主题](docs/images/genshin-night-live.jpg)

*真机截图：原神 · 星夜 轻量主题（无文字干净底图 + 自动配色），选择后会同步切换 Codex 深色外观。*

![真机截图：大佬点烟彩蛋主题](docs/images/dalao-live.jpg)

*真机截图：大佬 · 点烟 彩蛋主题（纯黑场 + 火光琥珀配色）。*
...
只有 `schemaVersion`、`id`、`name` 和 `hero` 必填。图片必须位于主题目录内，颜色、文案、`appearance` 和 `previewFocus` 都可省略；`appearance` 可设为 `light`、`dark` 或 `system`。`previewFocus` 的 `x`、`y` 使用 0 到 100 的整数，控制主题中心「当前主题」横幅的主体焦点，不改变 Codex 全屏背景构图。
...
![外观主题配色设置](docs/images/appearance-theme-contrast.jpg)
...
本仓库前身走 ASAR 修改路线，当前实现已改为 CDP。远程 `v4.0.0` 与 `v5-asar-legacy` 在 2026-07-16 仍指向同一 commit，因此不能把后者当成已经核实的独立遗留快照；远程处置建议见 [审计加固处置报告](docs/release/2026-07-16-audit-hardening-disposition.md)。
...
**HeiGe Codex Skin Studio** reskins Codex Desktop through loopback CDP injection without modifying `app.asar`, binaries, or signature resources. A top menu switches themes, synchronizes Codex light or dark appearance, and controls next-launch persistence. The package also includes the optional `Miku Future` animated Pet without replacing built-in pets. Turning persistence off keeps the current session skinned and restores the native UI on the next launch. The local `HeiGe 皮肤启动器` and the compatibility `enable-skin` entry restore the last skin for the current session only. The top-menu switch is the only supported way to enable next-launch persistence.
